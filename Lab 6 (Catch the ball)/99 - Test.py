...
class Ball:
    ...
    def move(self):
        ...
        if self.x + self.r > WIDTH or self.x - self.r <= 0:
            self.x -= self.dx
            self.dx = -sign(self.dx) * randint(0, 5)
            if self.dx == 0:
                self.dy = -sign(self.dy) * randint(1, 5)
            else:
                self.dy = -sign(self.dy) * randint(0, 5)
        if self.y + self.r > HEIGHT or self.y - self.r <= 0:
            self.y -= self.dy
            self.dy = -sign(self.dy) * randint(0, 5)
            if self.dy == 0:
                self.dx = -sign(self.dy) * randint(1, 5)
            else:
                self.dx = -sign(self.dy) * randint(0, 5)
...
def add_ball():
    """Adding ball every n milliseconds"""
    global speed
    new_ball = Ball()
    balls.append(new_ball)
    if speed > 10: speed -= 5
    canvas.after(5000, add_ball)
